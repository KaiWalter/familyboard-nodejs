...
- [ ] T017 [US1] Implement `src/services/calendarService.js` fetch (placeholder stub returning sample events initially)
- [ ] T018 [P] [US1] Implement `src/server/routes/events.js` GET /api/events returning transformed events
- [ ] T019 [US1] Implement `public/js/calendarView.js` grid render (3 weeks x 7 columns) + hooks (.row-header, .column-header, .cell-header)
- [ ] T020 [P] [US1] Add month abbreviation logic (first Monday + day=1 cells) in calendarView.js
- [ ] T021 [US1] Add current day highlight logic + monochrome border/class `.current-day`
- [ ] T022 [US1] Add all-day vs timed event sorting & display format in calendarView.js
- [ ] T023 [US1] Integrate timezone conversion using util/timezone.js inside calendarService.js
- [ ] T024 [US1] Implement midnight rollover handler (setInterval comparison local day) in calendarView.js
- [ ] T025 [US1] Add empty state placeholder for no events day cells
- [ ] T026 [US1] Smoke test script `tests/smoke/calendar.test.mjs` verifying grid structure
...
- [ ] T044 [US4] Add test `tests/integration/tokenRefresh.test.mjs` mocking msal for success/failure/backoff
...
- [ ] T050 Document security guidance in README (token file permissions)
